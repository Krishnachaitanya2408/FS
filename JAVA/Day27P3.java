...
public class Day27P3{
    public static void main(String... args){
        ...
        for(int i=0;i<n-1;i++) from[i] = sc.nextInt();
        for(int i=0;i<n-1;i++) to[i] = sc.nextInt();
        for(int i=0;i<n-1;i++) w[i] = sc.nextInt();
        ...
        for(int i=0;i<n-1;i++){
            map.putIfAbsent(from[i], new ArrayList<>());
            map.get(from[i]).add(new int[]{to[i], w[i]});
            
            map.putIfAbsent(to[i], new ArrayList<>());
            map.get(to[i]).add(new int[]{from[i], w[i]});
        }
        
        System.out.println(map);
        
        //Incomplete code, need to implement the logic to find the number of valid pairs
    }
}