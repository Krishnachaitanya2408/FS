...
public class Day4P2{
    public static void main(String... args){
        ...
        int[] tasks = IntStream.range(0, n).map(i -> sc.nextInt()).toArray();
        int[] hours = IntStream.range(0, n).map(i -> sc.nextInt()).toArray();
        ...
        for(int i=0;i<n;i++){
            ...
            if(i>=k-1){
                if(T<lower && E>=work_goal) tot -= 2;
                else if(T<lower && E<work_goal) tot -= 1;
                else if(T>=upper && E>=work_goal) tot+=1;
                else if(T>=upper && E<work_goal) tot+=2;
                T -= tasks[l];
                E -= hours[l];
                l++;
            }
