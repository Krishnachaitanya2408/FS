...
public class Day54P1{
    ...
    public static void main(String... args){
        ...
        for(int i=0;i<a.length;i++) a[i] = Integer.parseInt(input[i]);
        
        System.out.println(find(0, a, 0));
    }
    
    public static int find(int i, int[] a, int c){
        if(i>=a.length-1) return c;
        
        for(int j=1;j<=a[i];j++){
            res = Math.min(res, find(i+j,a, c+1));
        }
