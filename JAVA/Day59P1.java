...
public class Day59P1
{
    public static void main(String[] args) 
    {
        ...
        Set<Integer> upsideSet = new HashSet<>();
        for (int i = 0; i < n; i++) 
        {
            upsideSet.add(up[i]);
        }
        ...
        {
            int currentUp = up[i];
            int currentDown = down[i];

            if (!upsideSet.contains(currentDown)) 
            {
                candidates.add(currentDown);
            }

            if (currentUp != currentDown) 
            {
                upsideSet.remove(currentUp);
                upsideSet.add(currentDown);

                if (!upsideSet.contains(currentUp)) 
                {
                    candidates.add(currentUp);
                }

                upsideSet.remove(currentDown);
                upsideSet.add(currentUp);
            }
        }
