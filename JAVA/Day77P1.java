...
class OrderProcessor implements Runnable {
   ...
   public void run() {
        ...
        try {
            ...
            String success = "Order accepted: " + order.getCustomerName() + " ordered "  + order.getQuantity() + " of " + order.getProductName();
            outputMap.put(indexedOrder.index, success);
        } catch (Exception e) {
            String failure = "Order failed: " + order.getCustomerName() + " - " + e.getClass().getSimpleName();
            outputMap.put(indexedOrder.index, failure);
