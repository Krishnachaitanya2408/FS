...
async function processBooking(userId) {
  ...
  const [moviesFetched, userVerified] = await Promise.all([
  ...
  const movieChoosed = await chooseMovie(moviesFetched);
  console.log(`Movie chosen: ${movieChoosed}`);
  
  const selectedShowtime = await selectShowtime(movieChoosed);
